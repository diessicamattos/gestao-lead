...
export default function AssignLeadDialog({
  ...
  useEffect(() => {
    ...
    (async()=>{
      ...
      setUsers(s.docs.map(d=>({uid: d.id, displayName: (d.data() as any).displayName})));
    })();
  }, [open]);
  ...
  return (
    <Dialog open={open} onClose={onClose}>
      ...
      <DialogContent>
        <TextField select label="Vendedora" value={uid} onChange={e=>setUid(e.target.value)} fullWidth sx={{mt:1}}>
          {users.map(u=> <MenuItem key={u.uid} value={u.uid}>{u.displayName || u.uid}</MenuItem>)}
        </TextField>
      </DialogContent>
      <DialogActions>
        <Button onClick={onClose}>Cancelar</Button>
        <Button onClick={atribuir} variant="contained" disabled={!uid}>Atribuir</Button>
      </DialogActions>
    </Dialog>
  );
}
